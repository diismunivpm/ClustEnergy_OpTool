...
from MeteoFile import MeteoFile,Window_solar,Wall_solar
from Functions import optForm_build
from DHW_model import DHW_tank
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def SFH2006_air(Locality, day, month, duration, timestep, th_sp, th_tolerance_sup, th_tolerance_min, thermal_load, occupancy, internalGains, Qmax, ACH, GR, wall_absorption, radiative_part, convective_part):
    ...
    if thermal_load == 'cooling':
        for i in range (samples):
            if occupancy[i] > 0:
                ...
                Upper.append(float(th_sp[i])  + th_tolerance_sup) # meaning the shutdown of the cooling system (active cooling)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def SFH2006_floor(Locality, day, month, duration, timestep, th_sp, th_tolerance_sup, th_tolerance_min, occupancy, internalGains, Qmax, ACH, GR, wall_absorption, radiative_part, convective_part):     
    """Recall of variables useful for defining inputs. """
    T_est           =  MeteoFile(Locality,int(day),int(month),duration,timestep)[0]  # Outdoor temperature trends (°C)
    T_ground        =  MeteoFile(Locality,int(day),int(month),duration,timestep)[1]  # Ground temperature trends (°C)
    Diff_Whm2       =  MeteoFile(Locality,int(day),int(month),duration,timestep)[2]  # Diffuse solar radiation (Whm2)
    ...
    for i in range(0,samples): 
        ...
        Bub_opt[i+(3*samples),0] = M_max_floor[i] - 19
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def SFH2005_air(Locality, day, month, duration, timestep, th_sp, th_tolerance_sup, th_tolerance_min, thermal_load, occupancy, internalGains,  Qmax, ACH, GR, wall_absorption, radiative_part, convective_part):     
    """Recall of variables useful for defining inputs. """
    T_est           =  MeteoFile(Locality,int(day),int(month),duration,timestep)[0]  # Outdoor temperature trends (°C)
    T_ground        =  MeteoFile(Locality,int(day),int(month),duration,timestep)[1]  # Ground temperature trends (°C)
    Diff_Whm2       =  MeteoFile(Locality,int(day),int(month),duration,timestep)[2]  # Diffuse solar radiation (Whm2)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def SFH1990_air(Locality, day, month, duration, timestep, th_sp, th_tolerance_sup, th_tolerance_min, thermal_load, occupancy, internalGains,  Qmax, ACH, GR, wall_absorption, radiative_part, convective_part):     
    """Recall of variables useful for defining inputs. """
    T_est           =  MeteoFile(Locality,int(day),int(month),duration,timestep)[0]  # Outdoor temperature trends (°C)
    T_ground        =  MeteoFile(Locality,int(day),int(month),duration,timestep)[1]  # Ground temperature trends (°C)
    Diff_Whm2       =  MeteoFile(Locality,int(day),int(month),duration,timestep)[2]  # Diffuse solar radiation (Whm2)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def SFH1960_air(Locality, day, month, duration, timestep, th_sp, th_tolerance_sup, th_tolerance_min, thermal_load, occupancy, internalGains,  Qmax, ACH, GR, wall_absorption, radiative_part, convective_part):     
    """Recall of variables useful for defining inputs. """
    T_est           =  MeteoFile(Locality,int(day),int(month),duration,timestep)[0]  # Outdoor temperature trends (°C)
    T_ground        =  MeteoFile(Locality,int(day),int(month),duration,timestep)[1]  # Ground temperature trends (°C)
    Diff_Whm2       =  MeteoFile(Locality,int(day),int(month),duration,timestep)[2]  # Diffuse solar radiation (Whm2)
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
def DHW(Locality, day, month, duration, timestep,tank_temp,tank_tol,T_inlet,tank_volume,h_tank,l_tank,U_tank,Q_hp,occupancy,draw):
    """Recall of variables useful for defining inputs. """
    T_est           =  MeteoFile(Locality,int(day),int(month),duration,timestep)[0]  # Outdoor temperature trends (°C)
    samples =  int(duration/timestep) # number of total steps
    ...
    for n in range(samples):
        Qwater_draw.append(float(draw[n])*specific_heat*(tank_temp-T_inlet)/3600)
    
    surf_area    = 2*math.pi*(l_tank/2)*h_tank + 2*math.pi*(l_tank/2)**2
    """Calling DHW tank class. """
    tank = DHW_tank(tank_volume, surf_area, U_tank)
    
    """Configuring the state-space model representation for linear programming problem resolution. """
    tank.stateSpace_DHW() # state-space representation for DHW tank
    
    U = np.empty([2,samples]) # input vector of the state-space model formulation
   
    for r in range (samples):       
        U[0,r] = T_est[r]                  # Outdoor temperature (°C)
        U[1,r] = (-1)*Qwater_draw[r]                  # Loss due to water draw (Wh)

    Xo     = np.array([[tank_temp]]) # initial state of water temperature

    tank.sys_dis_forOp(tank.sys_tuple,timestep) # System discretization for linear resolution
    
    Ad = np.asarray(tank.sys_disc_A) # State matrix A of the discretized system written in SSM form
    Bd = np.asarray(tank.sys_disc_B) # Input matrix B of the discretized system written in SSM form
    Ed = np.asarray(tank.sys_disc_E) # State matrix E of the discretized system isolating the decision variable 

    Z     = optForm_build(samples, Ad, Xo, Bd, U, Ed, 0)[0] # Variable describing the state of the system containing the decision variable (e.g., thermal demand)
    M_max = optForm_build(samples, Ad, Xo, Bd, U, Ed, 0)[1] # Variable describing the state of the indoor air temperture
    M     = optForm_build(samples, Ad, Xo, Bd, U, Ed, 0)[2]    
    ...
    for i in range(0,samples):
        # Twater <= Upper
        Aub_opt[i,0:(samples)] = Z[i,:]
        Bub_opt[i,0]           = float(tank_temp + tank_tol) - M_max[i]      
    for i in range(0,samples): 
        # Twater >= Lower
        Aub_opt[i+(1*samples),0:(samples)] = Z[i,:]*(-1)
        Bub_opt[i+(1*samples),0]           = M_max[i]-float(tank_temp)
    ...
    return(c,      # 0 - decision variable coefficients
           Aub,    # 1 - Matrix of inequality constraints
           Bub,    # 2 - Vector of inequality constraints
           bounds, # 3 - Decision variable bounds
           M,      # 4 - Variable describing the state of the water temperture
           Z,      # 5 - Variable describing the state of the system containing the thermal demand for DHW (decision variable)
           M_max)  # 6 - Variable describing the state of the water temperture
